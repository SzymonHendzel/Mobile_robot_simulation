#w pliku tym zdefiniowane są funkcje robota i w jakie stany wchodzi w poszczególnych akcjach 
...
class RobotCommandHandle(adpt.RobotCommandHandle):
    #definicja paramterów związanych z robotem
    def __init__(self,
                 name,
                 fleet_name,
                 config,
    ...
    def sleep_for(self, seconds): #funkcja powodująca zatrzymanie się robota na x czasu 
        goal_time =\
    ...
    #Funkcja służąca to przeplanowania trasy
    def replan(self): 
    ...
    #funkcja odpowiadająca za załadowanie nowej trasy do robota(planowanie odbywa się w module którego nie można zmienić)
    def follow_new_path(
            self,
            waypoints,
    ...
    def get_position(self): # zwaraca pozycję robota
    ...
    def get_battery_soc(self): #nie będzie używana
    ...
    def update(self):#uaktualnienie stanu robota
        while rclpy.ok():
    ...
    def get_current_lane(self): # zwracja informację na jakiej drodze znajduję się robot
    ...
    def get_speed_limit(self, target_waypoint):#dostosowuję prdędkość robota do prędkości lini
    ...
    def filter_waypoints(self, wps: list): #funkcja pomocnicza pomagająca w poruszaniu się pomiędzy wierzchołkami grafu
    ...
    def complete_robot_action(self):# zwraca informację że robot skończył wykonywać zadanie
        with self._lock:
    ...
    def newly_closed_lanes(self, closed_lanes): #tej funkcji nie będziemy używać wykorzysywana przy drzwiach i windach
    ...
    def dock_summary_cb(self, msg): #funkcja nie wykorzystywana
        for fleet in msg.docks:
            if(fleet.fleet_name == self.fleet_name):
                for dock in fleet.params:
                    self.docks[dock.start] = dock.path

    def mode_request_cb(self, msg):#funkcja nie wykorzystywana
